    ...
    try {
      ...
      const newUser = await prisma.externalWebhookHotmartReceiver.create({
        data: {
          ...
          purchaseApprovedDate: parseDate(data.data.purchase?.approved_date),
          ...
          purchaseOrderDate: parseDate(data.data.purchase?.order_date),
          purchaseDateNextCharge: parseDate(data.data.purchase?.date_next_charge),
          ...
          paymentPixExpirationDate: parseDate(data.data.purchase?.payment?.pix_expiration_date),
